...
function extractTranslatableKeys(): string[] {
  ...
  const translatableFields = [
    ...
    // Other
    'heroBadge', 'requirements', 'copyright',
    // Thank you page
...
function localDataPlugin() {
  return {
      ...
      server.middlewares.use((req: IncomingMessage, res: ServerResponse, next: Connect.NextFunction) => {
          ...
          req.on('end', () => {
            try {
              ...
              if (code === 'en') {
                res.statusCode = 400;
                res.setHeader('Content-Type', 'application/json');
                res.end(JSON.stringify({ error: 'Cannot delete English language' }));
                return;
              }
          ...
          parseRequestBody(req)
            .then(async (body) => {
              try {
                ...
                // OpenCode doesn't require API key
                const hasAuth = headers?.['Authorization'] || headers?.['x-goog-api-key'];
                const isOpenCode = endpoint?.includes('opencode.ai');
        ...
        // POST /__save-blog-post - Create/update blog post (dev only)
          ...
          parseRequestBody(req)
            .then((body) => {
              try {
                const { slug, frontmatter, content, lang } = JSON.parse(body);
    ...
    writeBundle() {
      ...
      fs.writeFileSync(langFilePath, JSON.stringify(languages));
  ...
  <lastmod>${now}</lastmod>
    ...
    ${jsonLd}`;
      ...
      if (!indexHtml.includes('rel="icon"')) {
        indexHtml = indexHtml.replace('</head>', '    <link rel="icon" type="image/svg+xml" href="/assets/svg/minios_icon.svg" />\n  </head>');
      }
