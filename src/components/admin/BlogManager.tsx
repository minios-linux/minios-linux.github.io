  ...
  const handleSavePost = async (
    ...
    try {
      ...
      if (editingPost) {
        ...
        result = await updatePost(editingPost.slug, {
          ...frontmatter,
          content
        } as Partial<BlogPost>, telegramConfig);
      ...
      // Handle Telegram publishing result
      if (result?.telegramPublished) {
        toast.success(
          telegramConfig?.delayMinutes && telegramConfig.delayMinutes >= 10
            ? t('Scheduled for Telegram in') + ` ${telegramConfig.delayMinutes} ` + t('minutes')
            : t('Published to Telegram')
        );
      } else if (result?.telegramError) {
        toast.error(t('Telegram error:') + ' ' + result.telegramError);
      }
        ...
        <DialogContent className={`blog-editor-dialog w-full max-h-[95vh] p-0 ${isPreviewMode ? 'blog-preview-mode' : ''}`}>
          ...
          <MarkdownEditor
            initialFrontmatter={editingPost ? {
              ...
              telegramPostId: editingPost.telegramPostId
            } : undefined}
